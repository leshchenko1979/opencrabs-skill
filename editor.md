...
```bash
git -C ~/opencrabs fetch adolfousier main && git -C ~/opencrabs merge --ff-only adolfousier/main
```

- Branch off fresh main; never off a stale checkout.
...
Phase 7 step 2b. The binary is live right here (`opencrabs-ops` user unit) —
no cargo needed.
...
#     Fork ci.yml was REMOVED 2026-08-26 (owner-ordered, cc100dc6 — on_push
#     zombie-run burn). The CARRIER replaces it as pre-flight:
gh workflow run quick-build-linux.yml --repo leshchenko1979/opencrabs \
  --ref ci/quick-build-linux -f source_repository=leshchenko1979/opencrabs \
  -f source_ref=<up-branch-tip-FULL-40-sha> -f features=<set-from-carrier-yml-default>
# Green = PR-head compile passes.
...
- **ATOMICITY (owner directive 2026-08-26):** issues, PRs and commits are atomic —
  ...
  branch.
- Pre-flight gate (step 2b) is MANDATORY (v0.4.0): read the fmt STEP outcome,
