...
2. **Issue Creation & Continuous Relationship Linking**: file a `fix(`/`bug(`-titled issue via `tools/issue/oc-issue-create` so its parent is carried ON the creating command (`gh issue create --parent <N>` is the raw equivalent); a declared orphan (`--no-parent "<reason>"`) is legal, a silent one is a violation.
