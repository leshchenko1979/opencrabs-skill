...
- **Resume idiom (previously codified nowhere).** A parked harvest resumes only if a lane remembers, so the mechanism is written down here:
  ...
  - **Known instances -- DERIVE them, never trust an enumerated list here.** Canonical read: `select name, id, enabled, cron_expr from cron_jobs where name like 'oc-harvest-%-resume'`. A list hand-written into this file is stale before it is committed: on 2026-09-19 this line named TWO instances while the live table held FOUR, the last created 19:08:23Z -- twelve minutes after the census it was written from, and a peer lane still filed a gap report off it. Snapshot 2026-09-19T19:10Z (names only, re-derive before acting): `oc-harvest-346-resume`, `oc-harvest-421-resume`, `oc-harvest-396-resume`, `oc-harvest-326-resume`.
