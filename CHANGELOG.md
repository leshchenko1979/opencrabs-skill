...
- **The scope-classifier specimen was wholly false.** I had cited `#531` as reproducing the negation trap. Re-tested at source: `#531`'s body contains `not tools/**` — a **bare string** matching **none** of the 12 `tools` keywords (`(tools):`, `tools/oc-`, `tools/lib/`, …), so it **never classified as `tools`**, pre-fix or post. The quoted title *"whole-corpus assertion re-derives its own predicate"* belongs to **no fork issue** among all 587. The Toolsmith's fix (#605) is correct and its blast radius is real — **6 of 181** issues change on prose tokens, every one a genuinely negated mention — but it did not fix the case I named, and the durable record now says so.
