...
| `./tools/oc-job-verify <run-id> <source-ref> [--features]` | standalone run-identity gate — oc-deploy poll inlines its own job-name decode | 0 VERIFIED / 2 IN-FLIGHT / 3 FAILED / 4 REF-MISMATCH / 5 NOT-FOUND |
...
| `./tools/oc-post-receipts ...` | standalone compiler-era manual fallback; oc-deploy swap receipts = journal + ledger + markers | 0 ok / 1 no-token / 2 send-failed / 3 bad-args |
...
| `./tools/oc-deploy <mode>` | the ship path itself — `ship` (fetch/push + 4 ORDER gates + carrier dispatch), `poll` (watch + RED scan + swap chain), `swap-execute` (Phase B swap), `watch [--with-delta]` (stray-commit tripwire; `--with-delta` appends the `oc-upstream-delta` advisory rows — merge B, v0.4.48), `contributors`/`fanout` (rows below); the editor's S3 ship path: `editor.md` §Phase 6 | 1 usage / 2 gate·push·verify·rollback fail / 4 stage-gate·launch fail / 9 kill file; per-mode rc see mode rows |
...
| `./tools/oc-watchdog-check` | **RETIRED v0.4.65** (lens E A1: pure passthrough to `oc-deploy watch`, zero added logic; DELETED — watchdog cadence calls `./tools/oc-deploy watch` directly; blob in git history) | — |
...
| `./tools/oc-commit -m <msg> [--issue N] [--repo <path>]` | gated commit wrapper (lens C5, v0.4.65): refuses detached HEAD, refuses unset `OC_ACTOR`, refuses empty index (NEVER stages anything), derives `Issue-Ref` from the actor's latest ledger claim (override `--issue`); adds `Session-Id` + `Issue-Ref` trailers | 0 committed / 2 usage / 3 gate-fail / 4 git-fail |
...
| `./tools/oc-wt add\|remove\|list <task> ...` | editor Phase 2 worktree ritual: `add` = prune → fetch → validate local branch → behind-base gate → worktree add → CHAINED `oc-index-worktree` (un-skippable); `remove` = dirty-tree gate, `--force` journals the destroyed listing FIRST; `list` = TSV task/path/branch/dirty-n over the repo's oc-wt trees (lens C10, v0.4.65); validates branches, never creates them | 0 ok / 2 usage / 3 path-exists-or-dirty / 4 index-failed / 5 repo-or-branch-missing / 6 behind-base |
