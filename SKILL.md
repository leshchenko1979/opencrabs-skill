...
| `./tools/oc-deploy <mode>` | the ship path itself — `ship` / `poll` / `swap-execute` / `status [--json]` / `watch [--with-delta]` / `fanout` / `contributors` RETIRED (use `oc-attrib --contributors`). Editor S3 path: `editor.md` §Ship. Verdict codes + wait semantics: tools/docs/RC-CONTRACT.md |
...
| `./tools/oc-health [--class <name>|--all] [--rotate] [--status] [--json] [--reap] [--quiet] [--selftest]` | 8-class rotating fleet health & cleanliness sweep (owner order 2026-09-11); spec + per-check remediation in `tools/docs/HEALTH-CLASSES.md` & `tools/docs/HEALTH-CHECKS.md`. `--reap` applies SAFE remediations only; without it the tool is a pure read |
