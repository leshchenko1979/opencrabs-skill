...
- **Canonical smoke log — ABSOLUTE PATH, stamp it literally (QUIRK from lane 6cd8175f, 2026-09-12):** `/root/.opencrabs/profiles/ops/opencrabs-dev/smoke-verdicts.log`. Every `smoke-verdicts.log` reference in this file and in `editor.md`/`triage.md` means THAT path. A bare filename resolved from the ops profile root hit a stale pre-move decoy at `/root/.opencrabs/profiles/ops/smoke-verdicts.log` and silently captured 2 rows (c10cd97b 22:38:00Z; 6cd8175f 09:09:39Z BLOCKED-INFRA) — both already SUPERSEDED in canonical (the BLOCKED-INFRA by a 09:46:44Z PASS; the 22:38:00Z row was a pre-screenshot draft), so neither was merged. Decoy RETIRED 2026-09-12: archived to `incident-evidence-20260912/smoke-verdicts.log.decoy-20260912.bak` and the old path now SYMLINKS to canonical, so a wrong-path write self-heals instead of being lost. **TWO artifacts, TWO writers — corrected 2026-09-12 (v0.4.163 conflated them; defect filed by lanes 52058a75 + 63d775f9, both verified first-hand):** (1) the **IDENTITY EVIDENCE BLOCK** is written by `oc-smoke-evidence --append-log` (bare = the canonical absolute above; a wrong path is unrepresentable, M2-2) — it emits LIVE deployment identity (pid, exe_sha256, artifact_sha256, run_id, VERDICT — 10 tab-separated lines) and can only describe the binary RUNNING AT THAT MOMENT. (2) the **VERDICT ROW** — `ts \t VERDICT \t run= \t sha= \t actor= \t issue= \t target= \t evidence=` — is **AUTHORED BY THE LANE** and appended newline-safely. That is the normal case and it is **NOT** a violation. What the ban targets is a row written with plain shell `>>`, which is newline-unsafe and is what glued c10cd97b's rows — do not use it. **Measured 2026-09-13 — the load-bearing claim is STRUCTURAL, and no absolute count belongs in this clause.** Confirmed in production: **1 tool-written identity block vs lane-authored verdict rows** — `--append-log` has never written a verdict row because it cannot; it is deployment proof, not a verdict ledger. Do NOT quote an absolute count from this file: it GROWS, so the figure rots within the hour (this clause read "136 lines / 55 rows" and the file was 146 lines before the hour was out). **State the PREDICATE with the number, or state no number** — a count whose definition is not stated is an assertion wearing a measurement's clothes. This clause has now failed that twice: the original "98 rows" was an unmeasured estimate, and its own v0.4.164 correction ("55 verdict rows: 9 canonical / 46 free-text") was equally unreproducible — FOUR lanes plus HQ re-measured and none reproduced 55. The mechanism was a regex whose token set held the short words `NA`/`N/A`/`PARTIAL`/`BLOCKED` behind a bare `^\S+\s+`, so it counted PROSE and `evidence=` continuation lines as rows (56 matches, 42 of them over-matches); under a stated strict predicate (ts prefix + a verdict token in field 2) the log holds ~14, and the "9 canonical" reproduces only as strict 8-field rows whose verdict is exactly `PASS`.
- **LATE ENTRY is sanctioned and has a mechanism (ruling 2026-09-12, resolving the :145 tension lane 63d775f9 raised):** the exception at the append-discipline clause above is REAL — a lane MAY append a historical verdict row marked `LATE ENTRY`, carrying its on-record receipts (ledger n=, issue comment, `deployed.meta` identity). The ban on "hand-typed" covers the WRITE MECHANISM (plain shell `>>`), never the AUTHOR — a lane-authored row appended newline-safely (e.g. `printf '%s\n' ... >>`, or the tool) is the ordinary path. A LATE ENTRY must name why the same-turn duty was missed. The sanctioned-writer half (a `--late-entry`/`--sha` override so `oc-smoke-evidence` can emit a historical row) is Toolsmith's, dispatched 2026-09-12.
...
- **The exclusion set is DERIVED BY THE TOOL at gate time — never by hand, never carried in a lane's context.** The 21 paths listed above are **illustrative, not normative**: that list rots the moment a template is added or removed, and a lane reproducing it by hand is the defect this clause exists to prevent (owner ruling 2026-09-12: *"that should be purely mechanical"*). The gate computes the set from the tree itself, at the moment it runs, by resolving the compiled-in `include_str!` targets against `src/**/*.rs` — the derivation is a property of the repository, not a fact any lane memorises. **A lane that finds itself needing this list has found a missing tool half, not a missing fact: report it (defect #20) rather than reproducing the list.**
