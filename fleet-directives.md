...
- **Watcher timeout = resume, not verdict** (mermaid lane c6b1a539, #65): a watcher dying or timing out with the run still in_progress is neither RED nor drop — resume via one-shot `gh run view` + re-verify job-pin before adopting. **ONE watcher law (v0.4.120, owner-ordered — supersedes the earlier "detached ≥60s poller" sanction):** CI verdict waits = `oc-waiter arm` (preferred) or one-shot polls on wake; raw `gh run watch`, hand-rolled `nohup` pollers, and `oc-prchecks --wait` double-duty are banned outright.
...
- **Waiter verdicts are polled, not trusted to notify** (lane 61161247, #116/#117 RED ~2h late): on any owner nudge, poll all armed waiter journals directly; terminal journal lines with absent notify receipts are the known failure shape.
