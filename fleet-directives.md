...
- **New Features (`feat/*`):** Must sit and mature in the live running deployment for **≥24 hours post-swap** (counted strictly from the live **deployment / hot-swap timestamp** in `deployed.ts` / swap journal when the candidate commit was actually deployed to production, **NOT** from git commit timestamp or issue filing time) before being eligible for harvest into an upstream PR (`adolfousier/opencrabs`). This ensures multi-session stability, real-world edge-case exposure, and regression soak time on the live binary.
- **Bug Fixes (`fix/*`):** Harvested to upstream **immediately** upon passing the verified 4-leg smoke gate (zero maturation hold).
