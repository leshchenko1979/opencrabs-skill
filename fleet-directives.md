...
- **Escalation: there is NONE — `interrupt` is INERT (owner re-ruling 2026-09-19, fork issue #373).** Send `turn-end` (the default). If the target is mid-turn, the message QUEUES and drains at the target's next tool-loop boundary; that is the whole mechanism. `interrupt: true` is a **legacy alias — accepted but INERT**, never an escalation, and `now` is **RETIRED and FAILS the delivery outright** (`notify_policy.rs` returns `Err`). Do not reach for either to hurry a message; a `no wake observed` confirm verdict MEANS the target is mid-turn, not that the message was lost — never re-send on it.
