...
- **Watcher timeout = resume, not verdict** (mermaid lane c6b1a539, #65): a detached watcher dying or timing out with the run still in_progress is neither RED nor drop — resume via one-shot `gh run view` + re-verify job-pin before adopting. **No detached long-watchers for CI** (aaa8d8ae, five `gh run watch` tasks killed by daemon restarts, zero results each): use one-shot polls on wake.
