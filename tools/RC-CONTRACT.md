...
| oc-health | 0 | 2 | 0 clean / 1 findings (warn-class present) / 3 measurement-failure (a check could not be measured — never silently treated as clean) · OWNER-ORDERED hourly sweep (2026-09-11): 11 checks — stale `fanout.lock.*` (age ≥24h AND owner pid dead), orphan worktree dirs on disk vs `git worktree list`, ledger backup count (keep 3), state-dir bloat (>200MB), disk headroom (warn 85 / urgent 89 → points at `vds-servers/scripts/cleanup-unified.sh`), shared-checkout dirty + HEAD-vs-remote divergence, ledger/skill version consistency, cron liveness (law-carrying crons enabled) + DM-leak (`deliver_to` positive telegram id, or prompt hardcoding the owner DM — a BLANK `deliver_to` is NOT a leak, the cron briefs via `session_notify`), stale `/tmp/oc-*` (≥48h), session-DB size (>1GB), `tools.log` JSONL integrity (non-parsing lines counted + named, report-only — 2 exist: 383/443, hand-written prose evidence; a plain `jq -r` aborts there and the flood guard's tail scan stops, so the recipes read raw `-Rr 'fromjson?'`) · `--reap` applies ONLY the provably-dead SAFE class (stale locks, backups beyond keep older than 24h, stale tmp); worktrees are NEVER auto-removed (`oc-wt remove` has a dirty-tree gate a sweep must not bypass) · `--json` is the whole stdout contract (no chatter on stdout) · check/remediation catalogue: `tools/HEALTH-CHECKS.md` · overrides for hermetic testing: `OC_HEALTH_STATE|FORK|DB|TMP_GLOB|SKILL_DIR|WT_PATTERNS|SKIP_VERSION|LOG` · `--selftest` = 22 offline fixture assertions |
