...
| oc-waiter | 0 | 2 (arm: usage-OR-actor-refused, OC_ACTOR unset/'unknown', #115C) | arm: 0 armed / 3 notify-bin-unresolvable (`arm --run <id>` alone is valid C-A1 — run id IS the target, ref recorded as run:<id>) · _run: 0 delivered / 4 NOTIFY-FAILED (verdicts GREEN/RED/TIMEOUT/ESCALATED/ESCALATED-CHURN/FAILED-TOOL/AMBIGUOUS/SUPERSEDED — verdict-mapping law v0.4.130 ruling n=2369: GREEN/RED fire ONLY on the gate's stdout `VERDICT GREEN/RED` marker (run-state conclusion), rc alone never verdicts (false-RED-on-GREEN class, run 34409434135); tool-error rcs 2/4/7 + markerless 0/3 retry with poll-floor backoff; FAILED-TOOL only after k=3 CONSECUTIVE tool errors (env OC_WAITER_TOOLERR_CAP, default 3) — healthy poll resets streak; AMBIGUOUS = gate rc 8 NON-retryable #115B; SUPERSEDED = gate rc-4 carrying OC_PR_SUPERSEDED, NON-retryable identity mismatch, C-A1 v0.4.104) · sweep: 0 clean / 1 orphans-reported / 3 notify-bin-unresolvable · list: 0 |
